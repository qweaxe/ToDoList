...
## 2026-05-12: OverdueView 筛选延续功能

### 改动内容
- DayView 筛选激活时，点击"查看筛选范围全部 →"跳转到 OverdueView 并延续筛选条件
- DayView 无筛选时，按钮文案保持"查看全部 →"
- OverdueView 新增 DayViewFilter 筛选器，支持分类/等级/任务类型筛选
- OverdueView 筛选激活时显示"显示全部逾期 / 仅显示筛选结果" toggle 按钮（不清除筛选条件）
- 通过 useViewStore.overdueFilter 临时传递筛选状态，跳转后立即清空避免残留

### 修改的文件
- `src/hooks/use-view-store.ts` - 新增 overdueFilter 字段和 setOverdueFilter 方法
- `src/components/views/DayView.tsx` - handleViewAllOverdue 传递筛选状态，按钮文案区分筛选状态
